...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthV1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lengthV2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const angle = Math.acos(scalarProduct / (lengthV1 * lengthV2));
...
// Prime numbers are positive integers greater than 1 that have exactly two
// distinct positive divisors.
// Check divisors i (from 2 to number / 2). For number 23, i ranges 2-11.
// "Check all divisors" doesn't make sense.
...
function getSumOfNumbers(...args) {
  return Math.round(args.reduce((prev, curr) => prev + curr) * 100) / 100;
}
